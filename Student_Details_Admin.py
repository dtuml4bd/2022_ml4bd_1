...
class Student_Details:
    def __init__(self, window):
        self.window = window
        self.window.geometry("1196x730+0+0")
        ...
        img = img.resize((1196, 730))
        self.photoImg = ImageTk.PhotoImage(img)

        bg_img = ttk.Label(self.window, image=self.photoImg)
        bg_img.place(x=0, y=0, width=1196, height=730)

        ########-------------MAIN_FRAME-------------------
        main_frame = ttk.Frame(bg_img)
        main_frame.place(x=25, y=45, width=1145, height=640)

        ########-------------LEFT_FRAME-------------------
        left_frame = ttk.Labelframe(main_frame, text="Thông tin sinh viên")
        left_frame.place(x=10, y=10, width=578, height=620)
        ...
        class_student_frame.place(x=5, y=260, width=565, height=335)
        ...
        right_frame.place(x=590, y=10, width=546, height=620)
        ...
        table_frame.place(x=5, y=230, width=535, height=370)
        ...
        self.student_table.heading("roll", text="Số phòng")
        ...
        if self.var_dep.get() == "Chọn ngành" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                ...
                if answer > 0:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        "update student set course=%s, department=%s, Year=%s, Semester=%s, Name=%s, Division=%s, Roll=%s, Gender=%s, Dob=%s, Email=%s, Phone=%s, Address=%s, Teacher=%s, PhotoSample=%s where Student_id=%s ",
                        (
                            self.var_course.get(),
                            self.var_dep.get(),
                            self.var_year.get(),
